...
var compHtml = `<label for="unitSelect">Choose a unit:</label>
  <select name="unit" id="unitSelect">
...
{% manifest %} 
    ...
    "params": [ 
        ...
        "description": "Enter the unit type - either 'Length' or 'Mass'. This will determine the conversion options and the default - Kg for mass, Meter for distance" 
        }] 
    } 
{% endmanifest %} 
...
{% endif %}
   ...
   var conversion = " (Unit: {{ defaultUnit }})"
   var field = "{{ fieldLogicalName | escape }}"
   $("#" + field + "_label").append(conversion);
   $("#" + field + "_label").append(compHtml);
   $("#" + field + "_label").append(userInput);
   $("#userValue").on("change", convertFunction);
